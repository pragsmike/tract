...
;; The get-post-id-from-head-request function is now removed.
...
;; --- SIMPLIFIED: This function now ONLY checks the database. ---
  ...
  (let [urls (->> (slurp file)
    ...
    (doseq [url urls]
      (let [expected-filename (util/url->filename url)
        ...
        (cond
          (.exists pending-file)
          (println (str "\t-> Skipping URL, output file already exists in pending: " expected-filename))
          ...
          (try
            ...
            (let [html-content (fetch-html-with-retry! driver url)
              ...
              (pipeline/write-to-next-stage! html-content next-stage-name expected-filename)
